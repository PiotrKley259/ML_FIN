...
TRADING_CONFIG = {
    'correlation_window': 24,        # 2 ans en mois  correlation historique de plus de 0.7 sur la période
    'min_correlation': 0.9,          # Maintient haute corrélation
    'min_cointegration_pvalue': 0.05, # cointégration moyenne
    'zscore_entry': 2,
    'zscore_exit': 0.5,
    'max_position_size': 0.05,
    'stop_loss': 0.01,
    'min_observations': 60,          # 5 ans en mois 
    'test_size': 0.2,
    'lookback_window': 12,           # 1 an et demi en mois 
...
class PairSelector:
    ...
    def find_valid_pairs_at_date(self, data, current_date, lookback_months=36):
        """Trouve les paires valides à une date donnée sans look-ahead bias - CORRIGÉ pour données mensuelles"""
        ...
        if len(valid_tickers) > 100:
            ...
            top_tickers = ticker_counts.nlargest(100).index.tolist() #10000 marche super bien 
            valid_tickers = [t for t in valid_tickers if t in top_tickers]
            logger.info(f"Limited to top 500 tickers by data availability")
...
class PairsTrader:
    """Trader de paires avec logique de mean reversion corrigée"""
    ...
    def calculate_spread_zscore(self, series1, series2, window=12):  
        """Calcule le z-score du spread sans look-ahead bias - adapté pour données mensuelles"""
