...
def sliding_window_dl_prediction_with_lasso(
    ...
    include_date_as_feature : bool = False,
    **base_dl_params
) -> Dict[str, Any]:
    """
    Run a sliding-window deep learning pipeline with adaptive feature selection.

    The process uses:
      - A sliding window through time to simulate a walk-forward validation (train/test split over time).
      - Feature selection in each window using Lasso/ElasticNet/adaptive Lasso etc.
    ...
    lasso_alpha_range : tuple(float, float)
        Alpha parameter range for feature selection.
    ...
    **base_dl_params : dict
    ...
    print(f"\n=== DEEP LEARNING (PyTorch) avec PRÉSÉLECTION ===")
...
# --- PHASE 1: FEATURE SELECTION ---
        ...
        if enable_bayesian_tuning and (window_count % tune_frequency == 0):
            ...
            def objective(trial):
                ...
                layer1 = trial.suggest_categorical('layer1', [1024, 1536, 2048, 3072])      # 64-512 → 512-2048
                layer2 = trial.suggest_categorical('layer2', [8, 12, 16, 24, 32])        # 32-256 → 256-1024  
                layer3 = trial.suggest_categorical('layer3', [6, 8, 10, 12])         # 16-128 → 128-512
                layer4 = trial.suggest_categorical('layer4', [2048, 3072, 4096, 6144])          # 8-64 → 64-256

                if n_layers == 3:
                    layer_sizes = [layer1, layer2, 6, layer1 *4]
                elif n_layers == 4:
                    layer_sizes = [layer1, layer2, 8, layer4]
                else:
    ...
    print(f"\n=== RÉSULTATS GLOBAUX DEEP LEARNING avec SÉLECTION ===")
